...
import { TrackContext } from "./membraneWebRTCNode";
 ...
 */
export interface MembraneWebRTCRoomConfig {
...
export interface RoomCallbacks {
   ...
   */
  onJoinSuccess?: (participantId: string, participantsInRoom: [Participant]) => void;
  /**
   ...
   */
  onTrackReady?: (ctx: TrackContext, participant: Participant) => void;
  /**
   ...
   */
  onTrackAdded?: (ctx: TrackContext, participant: Participant) => void;
  /**
   * Called when some track will no longer be sent.
   *
   * It will also be called before {@link onPeerLeft} for each track of this peer.
   */
  onTrackRemoved?: (ctx: TrackContext, participant: Participant) => void;
  /**
   * Called each time peer has its track metadata updated.
   */
  onTrackUpdated?: (ctx: TrackContext, participant: Participant) => void;
  /**
 ...
 */
export class MembraneWebRTCNode {
  ...
  // private trackIdToMetadata: Map<string, any> = new Map();
  private idtoParticipant: Map<String, Participant> = new Map();
  private localParticipant: Participant = { id: "", metadata: {} };
  ...
  private ingressNode: MembraneWebRTCNode;
  private egressNode: MembraneWebRTCNode;

  private readonly callbacks: RoomCallbacks;

  constructor(config: MembraneWebRTCRoomConfig) {
    const { callbacks, rtcConfig } = config;
    this.callbacks = callbacks;
    this.rtcConfig = rtcConfig || this.rtcConfig;

    this.ingressNode = new MembraneWebRTCNode({
      rtcConfig: this.rtcConfig,
      callbacks: {
        onSendMediaEvent: this.callbacks.onSendMediaEvent,
        // onJoinSuccess
        // onJoinError
        onTrackReady: (ctx) => this.callbacks.onTrackReady(ctx, this.getTrackAuthor(ctx)),
        onTrackAdded: (ctx) => this.callbacks.onTrackAdded(ctx, this.getTrackAuthor(ctx)),
        onTrackRemoved: (ctx) => this.callbacks.onTrackRemoved(ctx, this.getTrackAuthor(ctx)),
        onTrackUpdated: (ctx) => this.callbacks.onTrackUpdated(ctx, this.getTrackAuthor(ctx)),
        
      }}
    )
  }

  private getTrackAuthor = (ctx: TrackContext): Participant => {
    return {id: "", metadata: {}}
  }
   ...
   * ```
   */
  public join = (participantMetadata: any): void => {
    try {
      this.localParticipant.metadata = participantMetadata;
      const mediaEvent = generateMediaEvent("join", {metadata: participantMetadata})
  ...
  public receiveMediaEvent = (mediaEvent: SerializedMediaEvent) => {
    ...
    switch (deserializedMediaEvent.type) {
      case "peerAccepted":
        ...
        Array.from(Object.entries(deserializedMediaEvent.data.trackIdToMetadata)).forEach(
          ([trackId, metadata]) => this.trackIdToMetadata.set(trackId, metadata)
        );
    ...
    switch (deserializedMediaEvent.type) {
      ...
      case "tracksAdded":
        ...
        Object.entries(data.trackIdToMetadata).forEach(([trackId, metadata]) => {
          ...
          this.trackIdToMetadata.set(trackId, metadata);
      ...
      case "peerUpdated":
        ...
        break;
      case "trackUpdated":
        if (this.getPeerIdFromTrackId(deserializedMediaEvent.data.trackId) === this.localPeer.id)
          return;
        const trackId = deserializedMediaEvent.data.trackId;
        const trackMetadata = deserializedMediaEvent.data.metadata;
        this.trackIdToMetadata.set(trackId, trackMetadata);
        const trackContext = this.trackIdToTrack.get(trackId)!;
  ...
  private getTrackIdToMetadata = () => {
    ...
    Array.from(this.getLocalTrackIdToMatadata().entries()).forEach(([trackId, metadata]) => {
  ...
  private onTrack = () => {
    return (event: RTCTrackEvent) => {
      ...
      if (this.getPeerIdFromTrackId(trackId) === this.localPeer.id) return;

      const metadata = this.trackIdToMetadata.get(trackId);
      const trackContext = {
        stream,
        track: event.track,
        trackId,
